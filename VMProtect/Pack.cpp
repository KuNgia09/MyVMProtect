...
void Pack::DealTLSCallback(DWORD imgBuff) {
	...
	/*DWORD tlsFileOffset=RvaToOffset((PVOID)imgBuff, tlsDataDirectory.VirtualAddress);
	if (tlsFileOffset < -1) {
		snprintf(szMsg, sizeof(szMsg), "%s RvaToOffset failed\n", __FUNCTION__);
